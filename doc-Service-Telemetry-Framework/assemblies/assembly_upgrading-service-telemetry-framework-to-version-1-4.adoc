...
[role="_abstract"]
To migrate from {Project} ({ProjectShort}) v1.3 to {ProjectShort} v1.4, you must replace the `ClusterServiceVersion` and `Subscription` objects in the `service-telemetry` namespace on your {OpenShift} ({OpenShiftShort}) environment.

.Prerequisites

* You have upgraded your {OpenShiftShort} environment to v4.8. {ProjectShort} v1.4 does not run on {OpenShiftShort} versions less than v4.8.
