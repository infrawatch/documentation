...
.Procedure
. Create a `ServiceTelemetry` object with the property `observabilityStrategy: none` in the `spec` parameter. The manifest shown will result in a default deployment of {ProjectShort} suitable for receiving telemetry from a single cloud with all metrics collector types.
+
