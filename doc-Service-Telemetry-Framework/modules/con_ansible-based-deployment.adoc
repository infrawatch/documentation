...
As of {OpenStack} Wallaby, you can preview the use of Ansible instead of Puppet for deploying {Project} ({ProjectShort}) components. The use of Ansible has the following advantages:
endif::[]
ifeval::["{build}" == "downstream"]
As of {OpenStack} 17.0, you can preview the use of Ansible instead of Puppet for deploying {Project} ({ProjectShort}) components. The use of Ansible has the following advantages:
endif::[]
