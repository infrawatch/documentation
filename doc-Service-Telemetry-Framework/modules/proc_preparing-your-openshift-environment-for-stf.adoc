...
The primary condition we need to satisfy is that when an ElasticSearch pod is scheduled, that a label matching `tuned.openshift.io/elasticsearch` is present, at which point one of the two profiles we've listed will be matched. No action is required by the administrator if the recommended Operator for ElasticSearch is used. If a custom deployed ElasticSearch is used with {ProjectShort} then be sure to add the `tuned.openshift.io/elasticsearch` label to any scheduled pods.
