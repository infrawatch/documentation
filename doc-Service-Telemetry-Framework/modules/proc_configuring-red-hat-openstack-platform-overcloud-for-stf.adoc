...
ifdef::include_when_161[]

If you want to collect data without using the server side of {ProjectShort}, complete the following task:

* xref:collecting-data-without-using-stf-server-side[]

[[collecting-data-without-using-stf-server-side]]
== Collecting data without using {ProjectShort} server side

To collect data without using {ProjectShort} server side, you must create a file that contains the connection configuration of the client side {MessageBus} for the overcloud, enable external endpoints, and deploy the overcloud.
...
. In the `stf-connectors.yaml` file, add the `MetricsQdrExternalEndpoint: true` parameter to enable external endpoints.
+
[source,yaml]
----
parameter_defaults:
...
* the `qdr-form-controller-mesh.yaml` file that enables the client side {MessageBus} to connect to the external endpoints
+
