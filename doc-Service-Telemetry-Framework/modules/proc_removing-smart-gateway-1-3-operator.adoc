...
. Delete the SmartGateway Custom Resource Definition (CRD). After removal of the CRD no data will flow to {ProjectShort} until the upgrade is completed and the Smart Gateway instances are reinstantiated:
+
[source,bash,options="nowrap"]
----
