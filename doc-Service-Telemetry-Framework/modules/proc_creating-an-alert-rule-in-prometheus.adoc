...
. Create a `PrometheusRule` object that contains the alert rule. The rule will be loaded into Prometheus by the Prometheus Operator:
+
----
oc apply -f - <<EOF
...
. To validate the rules have been loaded into Prometheus by the Operator, create a pod with access to `curl`:
+
----
oc run curl --generator=run-pod/v1 --image=radial/busyboxplus:curl -i --tty
----

. Then run `curl` to access the `prometheus-operated` service to return the rules loaded into memory:
+
----
...
. Validate the output shows the rules loaded into the `PrometheusRule` object, for example the output contains the defined `./openstack.rules`, then exit from the pod:
+
----
[ root@curl:/ ]$ exit
----
