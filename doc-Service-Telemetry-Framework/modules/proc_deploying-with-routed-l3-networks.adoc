...
Add the following configuration example under the `ExtraConfig` section for each Compute node. In this example, `internal_apiN` is the network that the Compute node `N` is connected to. `N` represents a value that is different from the default internal API network name.
