...
| `highAvailabilityEnabled` | `{true\|false}` | Enable high availability in {ProjectShort}. For more information, see <<high-availability>>. Default value is `false`.

| `storageEphemeralEnabled` | `{true\|false}` | Enable ephemeral storage support in {ProjectShort}. For more information, see <<ephemeral-storage>>. Default value is `false`.
|===
