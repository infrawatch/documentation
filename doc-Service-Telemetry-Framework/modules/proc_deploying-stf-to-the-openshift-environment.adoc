...
The core available parameters for a `ServiceTelemetry` manifest are:

|===

| `eventsEnabled` | `{true\|false}` | Enable events support in {ProjectShort}. Requires prerequisite steps to ensure ElasticSearch can be started. See <<subscribing-to-elastic-cloud-on-kubernetes-operator>>. Default value is `false`.

| `metricsEnabled` | `{true\|false}` | Enable metrics support in {ProjectShort}. Default value is `true`.
