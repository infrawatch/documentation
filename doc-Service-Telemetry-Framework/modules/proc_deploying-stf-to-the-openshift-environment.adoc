...
You must subscribe to the AMQ Certificate Manager Operator before you deploy the other components because the AMQ Certificate Manager Operator runs globally-scoped and is not compatible with the dependency management of Operator Lifecycle Manager when used with other namespace-scoped operators.
...
| `eventsEnabled` | `{true\|false}` | Enable events support in {ProjectShort}. Requires prerequisite steps to ensure ElasticSearch can be started. For more information, see <<subscribing-to-elastic-cloud-on-kubernetes-operator>>. The default value is `false`.

| `metricsEnabled` | `{true\|false}` | Enable metrics support in {ProjectShort}. The default value is `true`.


=======
| `highAvailabilityEnabled` | `{true\|false}` | Enable high availability in {ProjectShort}. For more information, see <<high-availability_advanced-features>>. Default value is `false`.

| `storageEphemeralEnabled` | `{true\|false}` | Enable ephemeral storage support in {ProjectShort}. For more information, see <<ephemeral-storage_advanced-features>>. Default value is `false`.
|===
