...
+
* The `resource_registry` configuration directly loads the collectd service because you are not including the `collectd-write-qdr.yaml` environment file for multiple cloud deployments.
ifdef::include_when_13,include_when_17[]
* Replace the `MetricsQdrConnectors.host` parameter with the value of `HOST/PORT` that you retrieved in xref:retrieving-the-qdr-route-address_assembly-completing-the-stf-configuration[].
...
ifndef::include_when_13[]
* Set the `CollectdSensubilityResultsChannel` to define the topic for collectd-sensubility events. Ensure that this value is the exact string format `sensubility/cloud1-telemetry`
endif::[]
