...
* Created the base configuration for {ProjectShort}. For more information, see xref:creating-the-base-configuration-for-stf_assembly-completing-the-stf-configuration[].
...
ifdef::include_when_16[]
<6> Define the topic for collectd-sensubility events. This should be the exact string format of `collectd/cloud1-notify`
endif::include_when_16[]
+
