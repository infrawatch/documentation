...
By default, {OpenStack} nodes send data through two data collectors; collectd and Ceilometer. These components each send telemetry data or notifications to the respective AMQP addresses, for example `collectd/telemetry`, where {ProjectShort} listens on those addresses for monitoring data.
