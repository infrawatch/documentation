...
* Configure the `ServiceTelemetry` object using `oc edit stf default` command and make the following changes:
+
[source,yaml]
----
...
* Configure the `ServiceTelemetry` object with the `oc edit stf default` command and make the following changes:
+
[source,yaml]
----
apiVersion: infra.watch/v1beta1
