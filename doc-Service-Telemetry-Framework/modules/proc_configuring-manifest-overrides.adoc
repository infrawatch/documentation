...
Edit the `ServiceTelemetry` object and provide a parameter and manifest. For a list of available manifest override parameters, see <<manifest-overrides_advanced-features>>. The default `ServiceTelemetry` object is `stf-default`. Use `oc get servicetelemetry` to list the available {ProjectShort} deployments.
