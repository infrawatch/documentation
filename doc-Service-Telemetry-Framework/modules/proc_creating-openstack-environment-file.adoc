...
. Create the `stf-connectors.yaml` and modify it to match the AMQP address you want for this cloud deployment:
+
.stf-connectors.yaml
----
resource_registry:
...
. Ensure that the naming convension in the `stf-connectors.yaml` aligns with the `spec.amqpUrl` field in the Smart Gateway configuration. For example, configure the `CeilometerQdrEventsConfig.topic` field to a value of `cloud1-event`.
