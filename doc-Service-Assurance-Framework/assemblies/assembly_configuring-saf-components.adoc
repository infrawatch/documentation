...
After you install the {Project} ({ProjectShort}) server-side components, configure the {ProjectShort} data collection to collect data and store it on the cloud platform side.  You must enable data collection within your {OpenStack} environment and direct it back to {ProjectShort}.  This process includes installing and configuring collectd.
