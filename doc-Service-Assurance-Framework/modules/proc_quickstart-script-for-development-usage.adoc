...
You can use a script to deploy SAF into an existing {OpenShift} environment for development purposes. It is not intended for
production environments. The script runs all of the required steps for a
single-cloud setup. Ensure that the following tools are installed before execution:
