...
The `send_queue_limit` parameter is used to limit the length of the queue to store outgoing metrics. If there is no
[NOTE] If there is no AMQP1 connection, the plugin continues to queue messages to send, which can result in unbounded memory consumption.
The default value is 0, which disables this feature. If metrics are missing, it may make sense to increase the parameter.
