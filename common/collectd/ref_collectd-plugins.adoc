...
Use the `apache` plugin to collect Apache data from the mod_status plugin provided by the Apache web server. Each instance provided is connected to per `interval` value specified in seconds. If provided, the `timeout` interval parameter for an instance is specified in milliseconds.
...
In the provided example, the instance name is `localhost` which connects to the Apache web server at http://10.0.0.111/mod_status?auto. You must append `?auto` to the end of the URL otherwise the status page will be returned as a type incompatible with the plugin.
...
The `csv` plugin will write values to a local file in CSV format.
...
The `smart` plugin collects SMART information from physical disks on the node.
...
The `swap` plugin collects information about the available and used swap space.
...
The `tcpconns` plugin collects information about the number of TCP connections inbound or outbound from the configured port. The local port configuration represents the inbound connections to the port. The remote port configuration represents the outbound connections from the port.
...
The `uptime` plugin collects information about system uptime. This plugin is enabled by default.
...
The `vmem` plugin collects information about virtual memory from the kernel subsystem.
