...
Reference the options tables of available parameters for specific plugins, such as in the following example:
----
ExtraConfig:
...
[NOTE]
If no interfaces are listed, then the default is for all interfaces to be monitored.
...
Use the `write_kafka` plugin to send values to a Kafka topic. Configure the `write_kafka` plugin with one or more topic blocks. For each topic block, you must specify a unique name and one Kafka producer. You can use the following per-topic options inside the topic block:
