...
| time
| The average time an I/O-operation takes to complete. Since this is a little messy to calculate take the actual values with a grain of salt.

| io_time
| Time spent doing I/Os (ms). You can make use of this metric as a device load percentage. A value of 1 second matches 100% of load.
...
The `interface` plugin is enabled by default. Use this plugin to measure interface traffic in octets, packets per second, and error rate per second.  of interfaces. 
